...
class GCNConv(nn.Module):
    def __init__(self,
        ...
        self.gcn_conv = GraphConv(in_feats, out_feats, bias=False)
    ...
    def forward(self, graph, feat, bond):
        ...
        self.gcn_conv(graph, feat, edge_weight = self.bond_encoder(bond))
        x = self.fc(feat)
...
class GCN(nn.Module):
    def __init__(self,
        ...
        # input layer
        self.layers.append(GCNConv(emb_dim, emb_dim, bias=False, allow_zero_in_degree=True))
    ...
    def forward(self, g, atom, bond):
        ...
        for i, layer in enumerate(self.layers[:-1]):
            x = layer(g, x, edge_weight=bond)
            x = self.bns[i](x)
            x = F.relu(x)
            x = self.dropout(x)
        x = self.layers[-1](g, x, edge_weight=bond)
